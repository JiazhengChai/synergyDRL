...
for agent in os.listdir(path_to_folder):
    ...
    for csv in os.listdir(path_to_agent):
        ...
        if agent=='HC':
            FSA_mean_HC=csv_file['FSA mean']
            FSA_std_HC= csv_file['FSA std']

            DSA_mean_HC = csv_file['DSA mean']
            DSA_std_HC = csv_file['DSA std']

            ASA_mean_HC = csv_file['ASA mean']
            ASA_std_HC = csv_file['ASA std']

        elif agent=='HCheavy':
            FSA_mean_HCheavy = csv_file['FSA mean']
            FSA_std_HCheavy = csv_file['FSA std']

            DSA_mean_HCheavy = csv_file['DSA mean']
            DSA_std_HCheavy = csv_file['DSA std']

            ASA_mean_HCheavy = csv_file['ASA mean']
            ASA_std_HCheavy = csv_file['ASA std']
        elif agent == 'FC':
            FSA_mean_FC = csv_file['FSA mean']
            FSA_std_FC = csv_file['FSA std']

            DSA_mean_FC = csv_file['DSA mean']
            DSA_std_FC = csv_file['DSA std']

            ASA_mean_FC = csv_file['ASA mean']
            ASA_std_FC = csv_file['ASA std']
...
ax_FSA.set_ylabel('Area(arbritary)', fontweight='bold')
...
plt.savefig(cwd+'/experiments_results/Synergy/histograms/SA_hitsogram.png')
