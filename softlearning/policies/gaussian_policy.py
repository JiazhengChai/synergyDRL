...
class GaussianPolicy(LatentSpacePolicy):
    def __init__(self,
        ...
        actions_std = tf.keras.layers.Lambda(
            lambda log_scale_diag: tf.exp(log_scale_diag)
        )(log_scale_diag)
        self.actions_std_model = tf.keras.Model(self.condition_inputs, actions_std)
    ...
    def deterministic_actions(self, conditions):
        return self.deterministic_actions_model(conditions)

    def actions_std(self, conditions):
        return self.actions_std_model(conditions)
